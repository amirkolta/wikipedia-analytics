...
describe 'Articles', type: :request do
  describe 'GET v1/articles/:title/month_views' do
    ...
    context 'with a single char month param' do
      ...
      before do
        allow(GetArticleMonthViews).to receive(:execute).and_call_original
      end

      it 'formats the month for the wikipedia client' do
        get endpoint, params: params

        expect(GetArticleMonthViews).to have_received(:execute).with(article: article, year: year, month: '03')
      end
    end
  end
end